...
// McpLogin implements [user_ifaceconnect.AuthServiceHandler].
func (a *authServiceImpl) McpLogin(ctx context.Context, req *connect.Request[user_iface.McpLoginRequest]) (*connect.Response[user_iface.McpLoginResponse], error) {
	var err error
	...
	source := access_iface.RequestSource{
		...
		RequestFrom: access_iface.RequestFrom_REQUEST_FROM_MCP,
	}
	...
	// set ke redis mcp session manager
	a.sessionManager.SetSession(ctx, pay.SessionId, &san_mcp.McpSession{
		Token:     token,
		PdcSource: xPdcSourceToken,
		UserId:    uint64(usr.ID),
		TeamId:    uint64(teamId),
	})

	return &connect.Response[user_iface.McpLoginResponse]{
		Msg: &user_iface.McpLoginResponse{
			Token:      token,
